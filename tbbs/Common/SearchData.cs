...
namespace SI4T.Templating
{
    ...
    public class SearchData
    {
        ...
        [XmlElement("publicationid")] public int PublicationId { get; set; }

        [XmlElement("schemaid")] public int SchemaId { get; set; }

        //16 for component, 64 for page
        [XmlElement("itemtype")] public int ItemType { get; set; }

        [XmlElement("parentsgid")] public int ParentSGId { get; set; }

        //All the parent, grandparent etc. SGs
        [XmlElement("sgid")] public List<int> StructureGroups { get; set; }
        ...
        public virtual List<string> ProcessPage(Page page)
        {
            ...
            if (page.IsIndexed() && page.PageTemplate.IsIndexed() && page.OrganizationalItem.IsIndexed())
            {
                ...
                {
                    this.Title = GetPageTitle(page.Title);
                }
                else
                {
        ...
        private string GetPageTitle(string title)
        {
            ...
            {
                string replacement = "";
                result = rgx.Replace(title, replacement);
            }
            return result;
        }

        /// <summary>
        /// Fetch Schema Id from Page eithor from first CP or first Region CP
        /// </summary>
        /// <param name="page"></param>
        /// <returns>SchemaId</returns>
        private int FetchSchemaId(Page page)
        {
            if (page.ComponentPresentations != null && page.ComponentPresentations.Count > 0)
            {
                return page.ComponentPresentations[0].Component.Schema.Id.ItemId;
            }
            else
            {
                if (page.Regions != null && page.Regions.Count > 0)
                {
                    var regionCps = page.Regions[0].ComponentPresentations;
                    if (regionCps != null && regionCps.Count > 0)
                    {
                        return regionCps[0].Component.Schema.Id.ItemId;
                    }
                }
            }
            return 0;
        ...
        /// <summary>
        /// Gather All Component Presentations from Page or Page Regions
        /// </summary>
        /// <param name="page"></param>
        /// <returns>List ComponentPresentation</returns>
        private List<ComponentPresentation> GatherAllCpsFromPage(Page page)
        {
            ...
            if (page.Regions != null && page.Regions.Count > 0)
            {
                cps.AddRange(GetCpsFromRegions(page.Regions));
            }
        ...
        /// <summary>
        /// Get Component Presentations from Regions Recursivly
        /// </summary>
        /// <param name="pageRegions"></param>
        /// <returns>List ComponentPresentation</returns>
        private List<ComponentPresentation> GetCpsFromRegions(IList<IRegion> pageRegions)
        {
            List<ComponentPresentation> cps = new List<ComponentPresentation>();
            foreach (var pageRegion in pageRegions)
            {
                if (pageRegion.ComponentPresentations != null && pageRegion.ComponentPresentations.Count > 0)
                {
                    cps.AddRange(pageRegion.ComponentPresentations);
                }

                if (pageRegion.Regions != null && pageRegion.Regions.Count > 0)
                {
                    cps.AddRange(GetCpsFromRegions(pageRegion.Regions));
                }
            }
        ...
        {
            return GetDcpIdentifier(cp.Component.Id.PublicationId, cp.Component.Id.ItemId,
                cp.ComponentTemplate.Id.ItemId);
        }
